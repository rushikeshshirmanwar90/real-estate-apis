...
// Mock notification service for testing (since we can't easily import the TypeScript module)
class MockNotificationService {
  static async notifyMaterialActivityCreated(materialActivity) {
    const startTime = Date.now();
    const notificationId = `mock_${materialActivity._id || 'unknown'}_${startTime}`;
    ...
    // Simulate structured logging with context
    const context = {
    ...
    console.log(`🔔 [Activity:${context.activityId}|User:${context.userId}|Client:${context.clientId}|Project:${context.projectId}|Type:${context.activityType}] Starting material activity notification processing`);

    // Validate material activity and generate appropriate errors
    const validationErrors = this.validateMaterialActivity(materialActivity);
    if (validationErrors.length > 0) {
      validationErrors.forEach(errorMsg => {
        const error = {
          type: 'VALIDATION_ERROR',
          message: errorMsg,
          context: { materialActivity },
          retryable: false,
          timestamp: new Date()
        };
        result.errors.push(error);
        console.error(`❌ [Activity:${context.activityId}|User:${context.userId}|Client:${context.clientId}|Project:${context.projectId}|Type:${context.activityType}] Validation error: ${errorMsg}`);
      });
      return result;
    }

    // Simulate recipient resolution
    if (Math.random() < 0.3) { // 30% chance of recipient resolution failure
      const error = {
        type: 'RECIPIENT_RESOLUTION',
        message: 'Failed to resolve recipients for notification',
        context: { 
      ...
      result.errors.push(error);
      console.error(`❌ [Activity:${context.activityId}|User:${context.userId}|Client:${context.clientId}|Project:${context.projectId}|Type:${context.activityType}] Recipient resolution failed`);
    } else {
      result.recipientCount = Math.floor(Math.random() * 5) + 1;
    }

    // Simulate delivery attempts
    if (result.recipientCount > 0) {
      if (Math.random() < 0.4) { // 40% chance of delivery failure
        const error = {
        ...
        result.errors.push(error);
        result.failedCount = result.recipientCount;
        console.error(`❌ [Activity:${context.activityId}|User:${context.userId}|Client:${context.clientId}|Project:${context.projectId}|Type:${context.activityType}] Delivery failed for ${result.recipientCount} recipients`);
      } else {
        result.deliveredCount = result.recipientCount;
        result.success = true;
        console.log(`✅ [Activity:${context.activityId}|User:${context.userId}|Client:${context.clientId}|Project:${context.projectId}|Type:${context.activityType}] Successfully delivered to ${result.deliveredCount} recipients`);
      }
    }

    // Simulate performance warning for slow processing
    if (result.processingTimeMs > 10000) {
      console.warn(`🐌 [Activity:${context.activityId}|User:${context.userId}|Client:${context.clientId}|Project:${context.projectId}|Type:${context.activityType}] PERFORMANCE WARNING: Processing took ${result.processingTimeMs}ms`);
    }

    return result;
  }

  static validateMaterialActivity(materialActivity) {
    const errors = [];
...
class NotificationErrorHandlingPropertyTest {
  ...
  async runSingleIteration(iteration) {
    ...
    if (iteration % 10 === 0) {
      ...
      // ~83% valid activities (normal flow with potential delivery failures)
    ...
    try {
      ...
      result = await Promise.race([
        MockNotificationService.notifyMaterialActivityCreated(materialActivity),
  ...
  async verifyProperties(materialActivity, result, processingTime, iteration) {
    ...
    this.verifyPerformanceWarnings(processingTime, logs, iteration, result);
  ...
  verifyPerformanceWarnings(processingTime, logs, iteration, result) {
    try {
