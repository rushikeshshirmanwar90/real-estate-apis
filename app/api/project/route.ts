  ...
  try {
    ...
    if (id) {
      ...
      let cacheValue = await safeRedisOperation(
        async () => await safeRedisGetCache(`project:${id}`),
        null
      );
      if (cacheValue) {
    ...
    let cacheValue = await safeRedisOperation(
      async () => await safeRedisGetCache(cacheKey),
      null
    );
    if (cacheValue) {
      cacheValue = JSON.parse(cacheValue);
    ...
    await safeRedisOperation(
      async () => await safeRedisSetCache(cacheKey, JSON.stringify(projects), 'EX', 86400),
      null
    );
...
export const POST = async (req: NextRequest) => {
  ...
  try {
    ...
    await safeRedisOperation(async () => {
      const keys = await safeRedisKeysCache(`projects:${body.clientId}*`);
      if (keys.length > 0) {
...
export const PATCH = async (req: NextRequest) => {
  ...
  try {
    ...
    await safeRedisOperation(async () => {
      await safeRedisDelCache(`project:${id}`);
      const keys = await safeRedisKeysCache(`projects:*`);
      if (keys.length > 0) {
        await safeRedisDelCache(...keys);
      }
    }, null);
