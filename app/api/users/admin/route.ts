...
export const GET = async (req: NextRequest) => {
  ...
  try {
    ...
    // Check cache
    const cachedData = await safeRedisGetCache(`admin:all`);
    if (cachedData) {
      const cacheValue = JSON.parse(cachedData);
    ...
    await safeRedisSetCache(`admin:all`, JSON.stringify(adminData), 'EX', 86400);
  ...
  try {
    ...
    if (updateData.email) {
      ...
      if (oldAdmin && !Array.isArray(oldAdmin)) {
        oldEmail = (oldAdmin as { email?: string }).email;
      }
    }
