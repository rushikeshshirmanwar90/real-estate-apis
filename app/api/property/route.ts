...
export const GET = async (req: NextRequest) => {
  ...
  try {
    ...
    // Check cache
    const cachedData = await safeRedisGetCache(`property:user:${userId}`);
    if (cachedData) {
    ...
    await safeRedisSetCache(`property:user:${userId}`, JSON.stringify(property), 'EX', 86400);
...
export const POST = async (req: NextRequest) => {
  ...
  try {
    ...
    // Invalidate cache
    await safeRedisDelCache(`property:user:${userId}`);
...
export const DELETE = async (req: NextRequest) => {
  ...
  try {
    ...
    const keys = await safeRedisKeysCache(`property:user:*`);
    if (keys.length > 0) {
